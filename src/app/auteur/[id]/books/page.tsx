    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 w-full max-w-[900px] mx-auto px-5 py-[120px]">
        ...
        <div className="flex gap-10 mb-[60px]">
          <Link href={`/auteur/${id}`} className="text-t4 font-semibold text-dark hover:text-primary transition-colors">
            Biographie
          </Link>
          ...
          {authorBooks.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5">
              {authorBooks.map((book) => (
                <BookCard key={book.id} book={book} size="md" showTitle myRating={userRatings.get(book.id) ?? null} />
              ))}
            </div>
          ) : (
